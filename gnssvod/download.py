...
from gnssvod.funcs.funcs import (check_internet, obsFileName, navFileName,
                                 nav3FileName, obs3FileName, datetime2doy,
                                 gpsweekday)
...
__all__ = [
    "get_rinex", "get_rinex3", "get_navigation", "get_clock", "get_sp3",
    "get_ionosphere"
]
...
def get_rinex(stationList,
    ...
    if internet == False:
        raise Warning(
            'No internet connection! | Cannot download RINEX file...')

    if Datetime == False:
        date_start = datetime.date(year=int(date_start[-4:]),
                                   month=int(date_start[-7:-5]),
                                   day=int(date_start[-10:-8]))
        if date_finish != None:
            date_finish = datetime.date(year=int(date_finish[-4:]),
                                        month=int(date_finish[-7:-5]),
                                        day=int(date_finish[-10:-8]))
        ...
        for date in dateList:
            ...
            if os.path.exists(fileName) == True:
                if os.path.exists(fileName[:-2]) == True:
                    ...
                    print(fileName +
                          " exists in working directory | Extracting...")
                    Archive(fileName).extractall(os.getcwd())
                    continue
            file_topath = os.path.join(directory, fileName)
            fileDir = [
                server_root, obsFileDir,
                str(date.year), doy,
                str(date.year)[-2:] + 'o', fileName
            ]  # file directory
            ftp = '/'.join(fileDir)
            # Download the file
            try:
                ...
                raise Warning("Requested file", fileName,
                              "cannot be not found!")


def get_navigation(stationList,
                   date_start,
                   date_finish=None,
                   period='day',
                   Datetime=False,
        ...
        for date in dateList:
            if date >= datetime.date(year=2016, month=1, day=1):
                ...
                try:
                    ...
                    print(
                        "| Requested navigation file", fileName,
                        "cannot be not found! | Checking for IGS Navigation File..."
                    )
                    try:
                        ...
                        fileDir = [
                            server_root, obsFileDir,
                            str(date.year), doy,
                            str(date.year)[-2:] + 'p', igsFileName
                        ]
                        ftp = '/'.join(fileDir)
                        print('Downloading:', igsFileName, end='')
                        url.urlretrieve(ftp, file_topath)
                        print(" | Download completed for", igsFileName,
                              " | Extracting...")
                        Archive(igsFileName).extractall(os.getcwd())
                    except:
                        raise Warning("IGS Navigation File", igsFileName,
                                      "cannot be not found!")
            else:
...
def get_sp3(sp3file, directory=os.getcwd()):
    ...
    if sp3file[-3:] == 'sp3':
        ...
        sp3Date = datetime.datetime.strptime(
            sp3file.split('_')[1], '%Y%j%H%M').date()
        gpsWeek, gpsWeekday = gpsweekday(sp3Date, Datetime=True)
        gpsWeek = str(gpsWeek)
        fileName = sp3file + ".gz"
    else:
        raise Warning(
            "sp3 filename must either end in .sp3 (gpsWeek < 2238) or .SP3 (gpsWeek >= 2238)"
        )
        sys.exit("Exiting...")
...
def get_clock(clockFile, directory=os.getcwd()):
    ...
    if clockFile[-3:] == 'clk':
        ...
        clockDate = datetime.datetime.strptime(
            clockFile.split('_')[1], '%Y%j%H%M').date()
        gpsWeek, gpsWeekday = gpsweekday(clockDate, Datetime=True)
        gpsWeek = str(gpsWeek)
        fileName = clockFile + ".gz"
    else:
        raise Warning(
            "clock filename must either end in .clk (gpsWeek < 2238) or .CLK (gpsWeek >= 2238)"
        )
        sys.exit("Exiting...")
    ...
    try:
        ...
        try:
            ...
            raise Warning("Requested file", fileName,
                          "cannot be not found in FTP server | Exiting")
...
def get_ionosphere(ionFile, directory=os.getcwd()):
    ...
    if internet == False:
        raise Warning(
            'No internet connection! | Cannot download ionosphere file')
