...
class Hemi:
    ...
    def patches(self):
        ...
        def plotpatch(dfrow):
            ...
            return (Rectangle([azimin, elemax],
                              azimax - azimin,
                              elemin - elemax,
                              fill=True))
    ...
    def add_CellID(self,
        ...
        if not aziname in df:
            raise ValueError(
                f"No column '{aziname}' in the dataframe, indicate which column should be used with azi='ColumnName'"
            )
        if not aziname in df:
            raise ValueError(
                f"No column '{elename}' in the dataframe, indicate which column should be used with ele='ColumnName'"
            )
        ...
        idf = idf.groupby('eleind', group_keys=False).apply(
            azicut)  # groupby will drop rows with eleind=NaN
        if idname in df:
...
def hemibuild(angular_resolution, cutoff=0):
    ...
    ringlims = np.arange(angular_resolution / 2, 90 - cutoff,
                         angular_resolution)
    ...
    for iring, outer_radius in enumerate(ringlims[1:]):
        ...
        ring_area = 2 * np.pi * (
            1 - np.cos(np.deg2rad(outer_radius))) - 2 * np.pi * (
                1 - np.cos(np.deg2rad(inner_radius)))
        ...
        cells.append(
            pd.DataFrame(
                data={
                    'azi':
                    np.linspace(azispan / 2, 360 - azispan / 2, numcells),
                    'ele':
                    np.full(numcells, 90 -
                            (inner_radius + angular_resolution / 2)),
                    'azimin':
                    azimin,
                    'azimax':
                    np.concatenate((azimin[1:], [360.0])),
                    'elemin':
                    np.full(numcells, 90 - outer_radius),
                    'elemax':
                    np.full(numcells, 90 - inner_radius)
                }))
