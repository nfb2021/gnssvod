...
class _Ellipsoid:
    ...
    def __init__(self, a, b):
        ...
        self.e1 = _np.sqrt(
            (a**2 - b**2) /
            a**2)  # first eccentricity  : $e   = \sqrt{\frac{a^2-b^2}{a^2}}$
        self.e2 = _np.sqrt(
            (a**2 - b**2) /
            b**2)  # second eccentricity : $e^' = \sqrt{\frac{a^2-b^2}{b^2}}$

    def radiusOfCurvature(self, phi):
        # Meridian radius of curvature (M)
        M = self.a * (1 - self.e1**2) / (
            1 - self.e1**2 * _np.sin(_np.deg2rad(phi))**2)**(
                3 / 2)  #$M = \frac{a(1 - e^2)}{(1 - e^2\sin(\phi)^2)^(3/2)}$
        # Prime vertical radius of curvature (N)
        N = self.a / (1 - self.e1**2 * _np.sin(_np.deg2rad(phi))**2)**(
...
def cart2ell(x, y, z, ellipsoid='GRS80'):
    ...
    lat_init = _np.arctan2(z, (1 - N_init * ellipsoid.e1**2 /
                               (N_init + h_init)) * p)
    while True:
        N = ellipsoid.a / _np.sqrt(1 -
                                   (ellipsoid.e1**2 * _np.sin(lat_init)**2))
...
def cart2ell_direct(x, y, z, ellipsoid='GRS80'):
    ...
    lat = _np.arctan2(z + ellipsoid.e2**2 * ellipsoid.b * _np.sin(beta)**3,
...
def ell2topo(lat, lon, h):
    ...
    north = _np.matrix([[-_np.cos(lon) * _np.sin(lat)],
                        [-_np.sin(lon) * _np.sin(lat)], [_np.cos(lat)]])
    up = _np.matrix([[_np.cos(lon) * _np.cos(lat)],
                     [_np.sin(lon) * _np.cos(lat)], [_np.sin(lat)]])
...
def geocentric_latitude(geodetic_latitude, ellipsoid='GRS80'):
    ...
    return _np.rad2deg(
        _np.arctan(_np.tan(_np.deg2rad(geodetic_latitude)) / (1 - ell.f)**2))
