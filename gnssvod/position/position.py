...
from gnssvod.geodesy.coordinate import _distance_euclidean
from gnssvod.position.atmosphere import tropospheric_delay
from gnssvod.position.satellite import _reception_coord, _sagnac, _azel, _relativistic_clock
from gnssvod.funcs.constants import (_SYSTEM_RNX2, _SYSTEM_RNX3, _SYSTEM_NAME,
                                     _CLIGHT)
# ===========================================================
...
def gnssDataframe(station, orbit, system="G+R+E+C+J+I+S", cut_off=7.0):
    ...
    gnss = _pd.concat(
        [station.observation.loc[epochMatch].copy(), orbit.loc[epochMatch]],
        axis=1)
    gnss = gnss[gnss['SYSTEM'].isin(system)]
    gnss["Distance"] = _distance_euclidean(station.approx_position[0],
                                           station.approx_position[1],
                                           station.approx_position[2], gnss.X,
                                           gnss.Y, gnss.Z)
    gnss["Relativistic_clock"] = _relativistic_clock(gnss.X, gnss.Y, gnss.Z,
                                                     gnss.Vx, gnss.Vy, gnss.Vz)
    gnss['Azimuth'], gnss['Elevation'], gnss['Zenith'] = _azel(
        station.approx_position[0], station.approx_position[1],
        station.approx_position[2], gnss.X, gnss.Y, gnss.Z, gnss.Distance)
    gnss = gnss.loc[gnss['Elevation'] > cut_off]
    gnss["Tropo"] = tropospheric_delay(station.approx_position[0],
                                       station.approx_position[1],
                                       station.approx_position[2],
                                       gnss.Elevation, station.epoch)
    return gnss
...
def multipath(station, system="G"):
    if len(system) > 1:
        raise Warning(
            "Multiple satellite system is not applicable for multipath | This feature will be implemented in next version."
        )
    observation_list = _observation_picker(station, system=system)
    observation = station.observation.dropna(subset=[
        observation_list[0][2], observation_list[1][2], observation_list[0][3],
        observation_list[1][3]
    ])
    observation = observation.loc[observation.SYSTEM ==
                                  _SYSTEM_NAME[system]].copy(deep=True)
    ...
    observation["Multipath1"] = pseudorange1 - (2 / (ioncoeff - 1) + 1) * (
        carrierPhase1 * lam1) + (2 / (ioncoeff - 1)) * (carrierPhase2 * lam2)
    observation["Multipath2"] = pseudorange2 - (
        2 * ioncoeff /
        (ioncoeff - 1)) * (carrierPhase1 * lam1) + (2 * ioncoeff /
                                                    (ioncoeff - 1) -
                                                    1) * (carrierPhase2 * lam2)
    ...
    for sv in sv_list:
        ...
        for i in range(1, len(ObsSV)):
            if (ObsSV.iloc[i].epoch - ObsSV.iloc[i - 1].epoch
                ) > _pd.Timedelta('0 days 00:15:00'):
                multipath1 = ObsSV.iloc[j:i].Multipath1.values - _np.nanmean(
                    ObsSV.iloc[j:i].Multipath1.values)
                multipath2 = ObsSV.iloc[j:i].Multipath1.values - _np.nanmean(
                    ObsSV.iloc[j:i].Multipath1.values)
                multipathSV1.extend(multipath1)
                multipathSV2.extend(multipath2)
                j = i
        multipath1 = ObsSV.iloc[j:].Multipath1.values - _np.nanmean(
            ObsSV.iloc[j:].Multipath1.values)
        multipath2 = ObsSV.iloc[j:].Multipath1.values - _np.nanmean(
            ObsSV.iloc[j:].Multipath1.values)
        multipathSV1.extend(multipath1)
        multipathSV2.extend(multipath2)
...
def _adjustment(coeffMatrix, LMatrix):
    ...
    m0 = _np.sqrt(
        _np.dot(_np.transpose(vMatrix), vMatrix) /
        (len(LMatrix) - len(NMatrix)))
...
def _observation_picker_by_band(station, system="G", band="L1"):
    ...
    try:
        ...
        if band not in _SYSTEM_RNX3[system].keys():
            raise Warning(band, "band cannot be found in", system,
                          "satellite system! Band options for", system,
                          "system:", tuple(_SYSTEM_RNX3[system].keys()))
    except KeyError:
        raise Warning("Unknown Satellite System:", system,
                      "OPTIONS: G-R-E-C-J-R-I-S")
    #-------------------------------------------------------------------
            ...
            for channel in channel_list:
                if (band + channel
                    ...
                    obs_codes.append([
                        ...
                        ("C" + band[1] + channel),
                        _SYSTEM_RNX3[system][band]["Frequency"],
                        ("D" + band[1] + channel), ("S" + band[1] + channel)
                    ])
                    break
    # RINEX-2
    elif station.version.startswith("2"):
        ...
        if band in _SYSTEM_RNX2[system].keys():
            for code in _SYSTEM_RNX2[system][band]["Pseudorange"]:
                if code in system_observations:
                    obs_codes.append([
                        system, band, band,
                        code, _SYSTEM_RNX2[system][band]["Frequency"],
                        ("D" + band[1]), ("S" + band[1])
                    ])
                    break
    return (obs_codes[0])
