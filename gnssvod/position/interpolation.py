...
from gnssvod.funcs.funcs import (sp3FileName, clockFileName, ionFileName,
                                 coord_interp)
...
def sp3_interp_fast(start_time,
                    end_time,
                    interval=30,
                    poly_degree=16,
                    sp3_product="gfz",
    ...
    start_epoch = _dt.date(start_time_withbuff.year, start_time_withbuff.month,
                           start_time_withbuff.day)
    end_epoch = _dt.date(end_time_withbuff.year, end_time_withbuff.month,
                         end_time_withbuff.day)
    epochs = _pd.date_range(start_epoch, end_epoch, freq='d')
    sp3FileNames = [sp3FileName(x.date(), sp3_product) for x in epochs]
    clockFileNames = [
        clockFileName(x.date(), interval, clock_product) for x in epochs
    ]
    #--------------------------------------------------------------------------
    ...
    for sv in svList:
        ...
        if len(sp3_temp) > 3:
            ...
            sp3_temp_resampled['Vx'] = -sp3_temp_resampled['X'].diff(
                periods=-1) / interval
            sp3_temp_resampled['Vy'] = -sp3_temp_resampled['Y'].diff(
                periods=-1) / interval
            sp3_temp_resampled['Vz'] = -sp3_temp_resampled['Z'].diff(
                periods=-1) / interval
            sp3_resampled.append(sp3_temp_resampled)

    sp3_resampled = _pd.concat(sp3_resampled, keys=svList,
                               names=['SV']).reorder_levels(
                                   ['Epoch', 'SV']).sort_index(level='Epoch')
    #--------------------------------------------------------------------------
    ...
    for sv in svList_clk:
        ...
        if len(clock_temp) > 3:
            ...
            clock_temp_resampled = clock_temp_resampled.interpolate(
                method='cubic')
            clock_resampled.append(clock_temp_resampled)

    clock_resampled = _pd.concat(clock_resampled,
                                 keys=svList_clk,
                                 names=['SV']).reorder_levels(
                                     ['Epoch', 'SV']).sort_index(level='Epoch')
    #--------------------------------------------------------------------------
    ...
    '''
    finish = time.time()
    print("SP3 interpolation is done in", '{0:.2f}'.format(finish - start),
          'seconds')
    return sp3matched


def sp3_interp(epoch,
               interval=30,
               poly_degree=16,
               sp3_product="gfz",
               clock_product="gfz"):
    ...
    yes = yes.loc[(slice(
        _pd.Timestamp(
            datetime(epoch_yesterday.year, epoch_yesterday.month,
                     epoch_yesterday.day, 23, 0, 0)),
        _pd.Timestamp(
            datetime(epoch_yesterday.year, epoch_yesterday.month,
                     epoch_yesterday.day, 23, 59, 59))))]
    tom = tom.loc[(slice(
        _pd.Timestamp(
            datetime(epoch_tomorrow.year, epoch_tomorrow.month,
                     epoch_tomorrow.day, 0, 0, 0)),
        _pd.Timestamp(
            datetime(epoch_tomorrow.year, epoch_tomorrow.month,
                     epoch_tomorrow.day, 3, 0, 0))))]
    ...
    header = ['X', 'Y', 'Z', 'Vx', 'Vy', 'Vz']
    # --------------------------------------------------------------
    epoch_start = _pd.Timestamp(
        datetime(epoch_yesterday.year, epoch_yesterday.month,
                 epoch_yesterday.day, 23, 0, 0))
    epoch_step = timedelta(hours=3)
    epoch_stop = epoch_start + timedelta(hours=4)
    dti = _pd.date_range(start=_pd.Timestamp(
        datetime(epoch_yesterday.year, epoch_yesterday.month,
                 epoch_yesterday.day, 23, 30, 0)),
                         end=_pd.Timestamp(
                             datetime(epoch_tomorrow.year,
                                      epoch_tomorrow.month, epoch_tomorrow.day,
                                      2, 29, 59)),
                         freq=str(interval) + 'S')
    index = _pd.MultiIndex.from_product([svList, dti.tolist()],
                                        names=['SV', 'Epoch'])
    interp_coord = _pd.DataFrame(index=index, columns=header)
    interp_coord = interp_coord.reorder_levels(['Epoch', 'SV'])
    interp_coord = interp_coord.sort_index()
    while True:
        sp3_temp = sp3.loc[(slice(epoch_start, epoch_stop))].copy()
        sp3_temp = sp3_temp.reorder_levels(["SV", "Epoch"])
        epoch_interp_List = _np.zeros(shape=(int(10800 / interval), 6,
                                             len(svList)))
        for svIndex, sv in enumerate(svList):
            if sv in sp3_temp.index.get_level_values('SV'):
                epoch_number = len(sp3_temp.loc[sv])
            ...
            if epoch_number <= poly_degree:
                print("Warning: Not enough epochs to predict for satellite",
                      sv, "| Epoch Count:", epoch_number,
                      " - Polynomial Degree:", poly_degree)
                epoch_interp_List[:, :, svIndex] = _np.full(shape=(int(
                    10800 / interval), 6),
                                                            fill_value=None)
                continue
            if epoch_number != 17:
            ...
            # Fit sp3 coordinates to 16 deg polynomial
            fitX = _np.polyfit(fitTime,
                               sp3_temp.loc[sv].X.copy(),
                               deg=poly_degree)
            fitY = _np.polyfit(fitTime,
                               sp3_temp.loc[sv].Y.copy(),
                               deg=poly_degree)
            fitZ = _np.polyfit(fitTime,
                               sp3_temp.loc[sv].Z.copy(),
                               deg=poly_degree)
            # Interpolate coordinates
            x_interp = coord_interp(fitX, interval) * 1000  # km to m
            x_velocity = _np.array([
                (x_interp[i + 1] - x_interp[i]) / interval if
                (i + 1) < len(x_interp) else 0 for i in range(len(x_interp))
            ])
            y_interp = coord_interp(fitY, interval) * 1000  # km to m
            y_velocity = _np.array([
                (y_interp[i + 1] - y_interp[i]) / interval if
                (i + 1) < len(y_interp) else 0 for i in range(len(y_interp))
            ])
            z_interp = coord_interp(fitZ, interval) * 1000  # km to m
            z_velocity = _np.array([
                (z_interp[i + 1] - z_interp[i]) / interval if
                (i + 1) < len(z_interp) else 0 for i in range(len(z_interp))
            ])
            sv_interp = _np.vstack(
                (x_interp[:-1], y_interp[:-1], z_interp[:-1], x_velocity[:-1],
                 y_velocity[:-1], z_velocity[:-1])).transpose()
            epoch_interp_List[:, :, svIndex] = sv_interp
            fitTime = _np.linspace(
                0, deltaT.seconds * 16,
                17)  # restore original fitTime in case it has changed!
        interp_coord.loc[(slice(epoch_start +
                                timedelta(minutes=30), epoch_stop -
                                timedelta(minutes=30, seconds=1))),
                         ('X', 'Y', 'Z',
                          'Vx', 'Vy', 'Vz')] = epoch_interp_List.transpose(
                              1, 0, 2).reshape(6, -1).transpose()
        epoch_start += epoch_step
        epoch_stop += epoch_step
        if epoch_start == _pd.Timestamp(
                datetime(epoch_tomorrow.year, epoch_tomorrow.month,
                         epoch_tomorrow.day, 2, 0, 0)):
            break
    # -------------------------------------------------------------------------
    interp_coord = interp_coord.reorder_levels(['Epoch', 'SV']).astype(float)
    clock.index.name = 'SV'
...
def ionosphere_interp(station,
    ...
    lat, lon, ellHeight = cart2ell(station.approx_position[0],
    ...
    E = (1 - p) * (1 - q) * tec[:, IndexLat + 1, IndexLon] + p * (
        1 - q) * tec[:, IndexLat + 1, IndexLon + 1] + q * (
            1 - p) * tec[:, IndexLat, IndexLon] + p * q * tec[:, IndexLat,
                                                              IndexLon + 1]
    ...
    for i in range(12):
        ...
        while observationEpochList[j] < epochEnd:
            tecFinal[j] = ((epochEnd - observationEpochList[j]) /
                           (epochEnd - epochStart)) * E[i] + (
                               (observationEpochList[j] - epochStart) /
                               (epochEnd - epochStart)) * E[i + 1]
            j += 1
