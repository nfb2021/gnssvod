...
def read_navFile(navigationFile):
    ...
    while True:
        ...
        if len(nav[0][1]) == 2:
            ...
            if 79 < year < 100:
                ...
                raise Warning(
                    'Navigation year is not recognized! | Program stopped!')
        else:
            year = int(nav[0][1])
        month, day, hour, minute, second = int(nav[0][2]), int(nav[0][3]), int(
            nav[0][4]), int(nav[0][5]), int(nav[0][6][0])
        ...
        if GLONASS or SBAS:
            ...
            if PRN == PRN_old and epoch == epoch_old:
                ephemeris_list[-1] = [
                    PRN, epoch, clockBias, relFeqBias, transmissionTime, roota,
                    toe, m0, e, delta_n, smallomega, cus, cuc, crs, crc, cis,
                    cic, idot, i0, bigomega0, bigomegadot, x, y, z, vx, vy, vz,
                    ax, ay, az, health, freqNumber, operationDay
                ]
            else:
                svList.append(PRN)
            ...
            x, y, z, vx, vy, vz, ax, ay, az, health, freqNumber, operationDay = [
                np.nan for _ in range(12)
            ]
            if PRN == PRN_old and epoch == epoch_old:
                ephemeris_list[-1] = [
                    PRN, epoch, clockBias, relFeqBias, transmissionTime, roota,
        ...
        if len(nav) == 0:
            break
    columnNames = [
        "SV", "Epoch", "clockBias", "relFeqBias", "transmissionTime", "roota",
        "toe", "m0", "eccentricity", "delta_n", "smallomega", "cus", "cuc",
        "crs", "crc", "cis", "cic", "idot", "i0", "bigomega0", "bigomegadot",
        "x", "y", "z", "vx", "vy", "vz", "ax", "ay", "az", "health",
        "freqNumber", "operationDay"
    ]
    ...
    fileEpoch = datetime.date(year=year, month=month, day=day)
    f.close()  # close the file
    finish = time.time()  # Time of finish
    print("Navigation file ", navigationFile, " is read in",
...
def read_obsFile_v2(observationFile, header=False):
    ...
    while True:
        if 'RINEX VERSION / TYPE' in obsLines[line]:
            ...
            approx_position = obsLines[line][0:obsLines[line].index('A'
                                                                    )].split()
            ...
            start_date = obsLines[line][0:obsLines[line].index('TIME')].split(
            )[0:-1]
            start_date = datetime.datetime(
                *np.array(start_date, dtype=float).round().astype(int))
            line += 1
        elif 'TIME OF LAST OBS' in obsLines[line]:
            end_date = obsLines[line][0:obsLines[line].index('TIME')].split(
            )[0:-1]
            end_date = datetime.datetime(
                *np.array(end_date, dtype=float).round().astype(int))
            line += 1
    ...
    if header:
        f.close()
        finish = time.time()
        print("Observation file ", observationFile, " is read in",
              "{0:.2f}".format(finish - start), "seconds.")
    ...
    while True:
        ...
        while True:
            if 'COMMENT' in obsLines[0]:
                ...
                del obsLines[0]
                line += 1
                print('Unexpected format between epochs! Line', line,
                      'is deleted!')
            else:
                break
        #---------------------------------------------------------------------------------------
        ...
        if 79 < year < 100:
            ...
            raise Warning(
                'Observation year is not recognized! | Program stopped!')
        epoch = datetime.datetime(year=year,
                                  month=int(obsLines[0][4:6]),
                                  day=int(obsLines[0][7:9]),
                                  hour=int(obsLines[0][10:12]),
                                  minute=int(obsLines[0][13:15]),
                                  second=int(obsLines[0][16:18])
                                  if isint(obsLines[0][16:18]) == True else 0)
        ...
        SV = [
            obsLines[0][32:][i:i + 3]
            for i in range(0, len(obsLines[0][32:]), 3)
        ]
        SV = [i.replace('  ', 'G0') for i in SV]
        SV = [
            i.replace(' ', 'G') if i[0] == ' ' else i.replace(' ', '0')
            for i in SV
        ]
        SVList.extend(SV)
        #---------------------------------------------------------------------------------------
        ...
        for i in range(0, rowNumber * NoSV, rowNumber):
            ...
            obsLines[i] = [
                float(obsLines[i][j:j + 14])
                if isfloat(obsLines[i][j:j + 14]) == True else None
                for j in range(0, 80 * rowNumber, 16)
            ]
    ...
    SVList = [x.replace(' ', '0') if x[1] == ' ' else x
              for x in SVList]  # replace 'G 1' with 'G01' (etc)
    ...
    observation["SYSTEM"] = _system_name(
        observation.index.get_level_values("SV"))
    #---------------------------------------------------------------------------------------
    ...
    print("Observation file ", observationFile, " is read in",
          "{0:.2f}".format(finish - start), "seconds.")
    return Observation(f.name, fileEpoch, observation, approx_position,
                       receiver_type, antenna_type, interval, receiver_clock,
                       version, ToB)
...
def read_obsFile_v3(obsFileName, header):
    ...
    if obsFileName.endswith("crx"):
        obsFileName = obsFileName.split(".")[0] + ".rnx"
            ...
            if obsLines[line][0] == "G":  # GPS
                ...
                if obsNumber_GPS > 13:
                    for _ in range(int(np.ceil(obsNumber_GPS / 13)) - 1):
                        ToB_GPS.extend(obsLines[line]
                                       [0:obsLines[line].index('SYS')].split())
                        line += 1
    ...
    ToB = np.unique(
        np.concatenate((ToB_GPS, ToB_GLONASS, ToB_GALILEO, ToB_COMPASS,
    ...
    observation_types = _ObservationTypes(ToB_GPS, ToB_GLONASS, ToB_GALILEO,
                                          ToB_COMPASS, ToB_QZSS, ToB_IRSS,
                                          ToB_SBAS)
    # --------------------------------------------------------------------------------------
    # if only the header was required, return a header object
    if header:
        f.close()
        finish = time.time()
        print("Observation file ", obsFileName, " is read in",
              "{0:.2f}".format(finish - start), "seconds.")
        return Header(f.name, approx_position, receiver_type, antenna_type,
                      start_date, end_date, version, ToB)
    # --------------------------------------------------------------------------------------
    ...
    while True:
        if time.time() - last_update > 30:
            ...
            print(
                f'Processed {currentline} out of {len(obsLines)} lines ({currentline/len(obsLines)*100:.1f}%)'
            )
        # =============================================================================
        while True:
        ...
        if obsLines[currentline][0] == ">":
            ...
            if len(epochLine) == 8:
                epoch_year, epoch_month, epoch_day, epoch_hour, epoch_minute, epoch_second, epoch_flag, epoch_SVNumber = obsLines[
                    currentline][1:].split()
                receiver_clock = 0
            elif len(epochLine) == 9:
                epoch_year, epoch_month, epoch_day, epoch_hour, epoch_minute, epoch_second, epoch_flag, epoch_SVNumber, receiver_clock = obsLines[
                    currentline][1:].split()
            else:
                raise Warning(
                    "Unexpected epoch line format detected! | Program stopped!"
                )
        else:
            if currentline + 1 == len(obsLines):
                # sometimes the last line is weird, especially if logging was interrupted
                print(
                    "The last line has an unexpected format and will be ignored"
                )
                break  # stop reading file
            else:
                print(
                    f"\033[93mUnexpected format detected! | Tentatively skipping line #{currentline}"
                )
                currentline += 1
        ...
        if epoch_flag in {"1", "3", "5", "6"}:
            ...
            for svLine in range(currentline, currentline + epoch_SVNumber):
                ...
                if obsLines[svLine].startswith("G"):
                    obsEpoch[0, index_GPS] = np.array([[
                        float(obsLines[svLine][3:][i:i + 14]) if isfloat(
                            obsLines[svLine][3:][i:i + 14]) == True else None
                        for i in range(0,
                                       len(ToB_GPS) * 16, 16)
                    ]])
                elif obsLines[svLine].startswith("R"):
                    obsEpoch[0, index_GLONASS] = np.array([[
                        float(obsLines[svLine][3:][i:i + 14]) if isfloat(
                            obsLines[svLine][3:][i:i + 14]) == True else None
                        for i in range(0,
                                       len(ToB_GLONASS) * 16, 16)
                    ]])
                    #obsEpoch[svLine,index_GLONASS]
                elif obsLines[svLine].startswith("E"):
                    obsEpoch[0, index_GALILEO] = np.array([[
                        float(obsLines[svLine][3:][i:i + 14]) if isfloat(
                            obsLines[svLine][3:][i:i + 14]) == True else None
                        for i in range(0,
                                       len(ToB_GALILEO) * 16, 16)
                    ]])
                elif obsLines[svLine].startswith("C"):
                    obsEpoch[0, index_COMPASS] = np.array([[
                        float(obsLines[svLine][3:][i:i + 14]) if isfloat(
                            obsLines[svLine][3:][i:i + 14]) == True else None
                        for i in range(0,
                                       len(ToB_COMPASS) * 16, 16)
                    ]])
                elif obsLines[svLine].startswith("J"):
                    obsEpoch[0, index_QZSS] = np.array([[
                        float(obsLines[svLine][3:][i:i + 14]) if isfloat(
                            obsLines[svLine][3:][i:i + 14]) == True else None
                        for i in range(0,
                                       len(ToB_QZSS) * 16, 16)
                    ...
                    obsEpoch[0, index_IRSS] = np.array([[
                        float(obsLines[svLine][3:][i:i + 14]) if isfloat(
                            obsLines[svLine][3:][i:i + 14]) == True else None
                        for i in range(0,
                                       len(ToB_IRSS) * 16, 16)
                    ...
                    obsEpoch[0, index_SBAS] = np.array([[
                        float(obsLines[svLine][3:][i:i + 14]) if isfloat(
                            obsLines[svLine][3:][i:i + 14]) == True else None
                        for i in range(0,
                                       len(ToB_SBAS) * 16, 16)
    ...
    print("Observation file ", obsFileName, " is read in",
          "{0:.2f}".format(finish - start), "seconds.")
    return Observation(f.name, fileEpoch, obs, approx_position, receiver_type,
                       antenna_type, interval, receiver_clock, version, ToB)
...
def read_sp3File(sp3file):
    ...
    header = [
        'X', 'Y', 'Z', 'deltaT', 'sigmaX', 'sigmaY', 'sigmaZ', 'sigmadeltaT',
        'Epoch'
    ]
    sat, pos = [], []
    while True:
        for i in range(int(SVNo) + 1):
            if '*' in sp3[i]:
                ...
                sp3[i] = [
                    float(j) if isfloat(j) == True else None for j in sp3[i]
                ]
                sp3[i].append(epoch)
                pos.append(sp3[i][1:])
    ...
    print('{}'.format(sp3file), 'file is read in',
...
def read_clockFile(clkFile):
    ...
    for i in range(len(timelist)):
        ...
        Epochlist.append(
            (datetime.datetime(year=int(timelist[i][2]),
    ...
    SVTimelist = pd.DataFrame(list(zip(Epochlist, SVtime)),
                              index=Sat,
                              columns=['Epoch', 'DeltaTSV'])
    f.close()  # close the file
    end = time.time()
    print('{}'.format(clkFile), 'file is read in',
          '{0:.2f}'.format(end - start), 'seconds')
    return SVTimelist
