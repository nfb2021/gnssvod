...
def clock_interp(fit, interval):
    ...
    Output:
        ...
        epoch:
    """
    epoch = _np.linspace(0, 86400,
                         int(86400 / interval) + 1)  # 86400 seconds = 24 hours
    time = _np.array([[epoch[t]**deg for deg in range(2, -1, -1)]
                      for t in range(int(86400 / interval) + 1)])
    interpClock = _np.array([[sum(fit * time[t, :])]
...
def coord_interp(parameter, interval):
    ...
    """
    epoch = _np.linspace(1800, 12600,
                         int(10800 / interval) +
                         1)  # 3h validity interval within 4h
