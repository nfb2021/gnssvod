...
def sp3FileName(epoch, product="gfz"):
    ...
    if gpsWeek < 2038:
        ...
        if timeDif.days == 0:
            raise Warning("Orbit files are not released yet for",
                          epoch.ctime())
            sys.exit("Exiting...")
...
def clockFileName(epoch, interval=30, product="cod"):
    ...
    if gpsWeek < 2038:
        ...
        if timeDif.days == 0:
            raise Warning("Clock files are not released yet for",
                          epoch.ctime())
            sys.exit("Exiting...")
        # we will force using GFZ for now..
        product = 'GFZ0MGXRAP'
...
def nav3FileName(stationName, date, zipped=False):
    ...
    if stationName.upper() == "BRDC":
        rinexFile = "BRDC00IGS_R_" + str(
            date.year) + str(doy) + "0000_01D_MN.rnx"
    else:
        rinexFile = siteInfo.SITE[0] + "_R_" + str(
            date.year) + str(doy) + "0000_01D_MN.rnx"
    """
    if len(doy) == 1:
...
def obs3FileName(stationName, date, zipped=False):
    ...
    siteInfo = IGS(stationName)
    rinexFile = siteInfo.SITE[0] + "_R_" + str(
        date.year) + str(doy) + "0000_01D_30S_MO.crx"
    if zipped == True:
        rinexFile = rinexFile + ".gz"
