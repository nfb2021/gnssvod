...
try:
    ...
    print("Warning: Basemap package cannot be imported |", e,
          " error detected - Groundtrack plot disabled!")
    _BasemapStatus = False
import matplotlib.patches as _mpatches
from gnssvod.geodesy.coordinate import cart2ell_direct
from gnssvod.position.position import gnssDataframe, multipath, _observation_picker_by_band
from gnssvod.position.interpolation import ionosphere_interp
...
def skyplot(station, orbit, system='G', SVlist=None, color=None):
    ...
    if color != None:
        color = color.lower()
        if color not in {
                'ionosphere', 'snr1', 'snr2', 'snr3', 'snr4', 'snr5', 'snr6',
                'snr7', 'snr8', 'snr9', 'multipath1', 'multipath2'
        }:
            raise Warning(
                color,
                "option for color mapping is not available for skyplot!")
    # ----------------------------------------------------------------------
    ...
    if color == 'multipath1' or color == 'multipath2':
        ...
        gnss = _pd.concat([
            gnss.loc[epochMatch].copy(), multipath_temp.loc[epochMatch].copy()
        ],
                          axis=1)
    ...
    if SVlist == None:
        ...
        for sv in SVlist:
            if sv[0] not in {'G', 'R', 'C', 'E'
                             } or sv[1:].isdigit() == False or len(sv) != 3:
                raise Warning(
    ...
    if len(SVlist) == 0:
        raise Warning(
            "Satellite(s) that you have entered not in SP3 file Program Stopped"
        )
    ...
    if color != None:
        ...
        if color == "ionosphere":
            gnss['Ion_Delay'] = ionosphere_interp(station,
                                                  unit='meter',
                                                  system=system,
                                                  band='L1',
                                                  epoch_list=gnss.epoch)
        for sv in SVlist:
            if color == 'ionosphere':
                ...
                obslist = _observation_picker_by_band(station,
                                                      system,
                                                      band='L' + color[-1])
    ...
    for sv in SVlist:
        if color == None:
            skyplot = ax.scatter(gnss.loc[sv].svx,
                                 gnss.loc[sv].Zenith,
                                 s=3,
                                 cmap=_mptl.cm.jet)
            ax.text(gnss.loc[sv].svx[int(len(gnss.loc[sv].svx) / 4)],
                    gnss.loc[sv].Zenith[int(len(gnss.loc[sv].Zenith) / 4)],
                    sv,
                    fontsize=9)
            ...
            ax.text(gnss.loc[sv].svx[int(len(gnss.loc[sv].svx) / 4)],
                ...
                'snr1', 'snr2', 'snr3', 'snr4', 'snr5', 'snr6', 'snr7', 'snr8',
                'snr9'
        }:
            obslist = _observation_picker_by_band(station,
                                                  system,
...
def azelplot(station, orbit, system='G', SVlist=None, color=None):
    if color != None:
        ...
        if color not in {
                'ionosphere', 'snr1', 'snr2', 'snr3', 'snr4', 'snr5', 'snr6',
                'snr7', 'snr8', 'snr9', 'troposphere'
        }:
            raise Warning(
                color,
                "option for color mapping is not available for azelplot!")
    # ----------------------------------------------------------------------
    gnss = gnssDataframe(station, orbit, system)
    gnss = gnss.reorder_levels(["SV", "Epoch"])
    ...
    if SVlist == None:
        ...
        for sv in SVlist:
            if sv[0] not in {'G', 'R', 'C', 'E'
                ...
                raise Warning(
                    '''Invalid format: Please enter satellite(s) that you want to plot proper format.
    ...
    for sv in SVlist:
        if color == None:
            _plt.scatter(gnss.loc[sv].Azimuth,
                         gnss.loc[sv].Elevation,
                         s=3,
                         cmap=_mptl.cm.jet,
                         lw=0,
                         vmin=minValue,
                         vmax=maxValue)
            _plt.text(gnss.loc[sv].Azimuth[int(len(gnss.loc[sv].Azimuth) / 3)],
                      gnss.loc[sv].Elevation[int(
                          len(gnss.loc[sv].Azimuth) / 3)],
                      sv,
                      fontsize=9)
            _plt.text(
                gnss.loc[sv].Azimuth[int(len(gnss.loc[sv].Azimuth) / 3 * 2)],
                gnss.loc[sv].Elevation[int(len(gnss.loc[sv].Azimuth) / 3 * 2)],
                sv,
                fontsize=9)
        elif color == 'ionosphere':
            _plt.scatter(gnss.loc[sv].Azimuth,
            ...
            _plt.text(gnss.loc[sv].Azimuth[int(len(gnss.loc[sv].Azimuth) / 3)],
                      gnss.loc[sv].Elevation[int(
                          len(gnss.loc[sv].Elevation) / 3)],
                      sv,
                      fontsize=9)
            _plt.text(gnss.loc[sv].Azimuth[int(
                len(gnss.loc[sv].Azimuth) / 3 * 2)],
                      gnss.loc[sv].Elevation[int(
                          len(gnss.loc[sv].Elevation) / 3 * 2)],
                      sv,
                      fontsize=9)
...
def bandplot(station, system='G', SVlist=None):
    ...
    for sv in SVlist:
        _plt.plot(gnss.loc[sv].Time,
                  gnss.loc[sv].Visibility,
                  linestyle='',
                  marker='.',
                  linewidth=1)
        #ax.xaxis.set_major_locator(mdates.HourLocator(byhour=range(24), interval=4))
        _plt.annotate(sv,
                      xy=(gnss.loc[sv].Time[0],
                          gnss.loc[sv].Visibility[0] + 20),
                      size='medium',
                      color='black')
...
def timelplot(station, orbit, system='G', SVlist=None, color=None):
    if color != None:
        color = color.lower()
        if color not in {
                'ionosphere', 'troposphere', 'snr1', 'snr2', 'snr3', 'snr4',
                'snr5', 'snr6', 'snr7', 'snr8', 'snr9', 'multipath1',
                'multipath2'
        }:
            raise Warning(
                color,
                "option for color mapping is not available for timelplot!")
    # ----------------------------------------------------------------------
    gnss = gnssDataframe(station, orbit, system)
    gnss = gnss.reorder_levels(["SV", "Epoch"])
    ...
    if color == 'multipath1' or color == 'multipath2':
        multipath_temp = multipath(station, system)
        epochMatch = gnss.index.intersection(
            multipath_temp.index)  # list of matched epochs and SVs
        gnss = _pd.concat([
            gnss.loc[epochMatch].copy(), multipath_temp.loc[epochMatch].copy()
        ],
                          axis=1)
    ...
    gnss['Time'] = gnss['epoch'].dt.time
    gnss['Seconds'] = _pd.to_timedelta(
        gnss['epoch']).apply(lambda x: x.total_seconds())
    ...
    for sv in SVlist:
        if color == None:
            _plt.scatter(gnss.loc[sv].Seconds,
                         gnss.loc[sv].Elevation,
                         s=3,
                         cmap=_mptl.cm.jet,
                         lw=0,
                         vmin=minValue,
                         vmax=maxValue)
            _plt.text(gnss.loc[sv].Seconds[int(len(gnss.loc[sv].Seconds) / 2)],
                      gnss.loc[sv].Elevation[int(
                          len(gnss.loc[sv].Elevation) / 2)],
                      sv,
                      fontsize=9)
        elif color == 'troposphere':
            _plt.scatter(gnss.loc[sv].Seconds,
    ...
    if color in {
            "ionosphere", "snr1", "snr2", "multipath1", "multipath2",
            "troposphere"
    }:
        _plt.colorbar(extend="both")
        figName = figName + " (" + color + ") for " + station.filename
...
if _BasemapStatus:
    ...
    def groundtrack(station, system='G', SVlist=None):
        gnss = station.observation
        SatEllCoor = cart2ell_direct(gnss.X.values, gnss.Y.values,
                                     gnss.Z.values)
        ...
        coloropt = [
            'violet', 'red', 'wheat', 'yellow', 'orange', 'magenta', 'lime',
            'ivory', 'gold', 'cyan', 'coral'
        ]
        ...
        _plt.title("Ground Track of GPS Satellites for {}".format(
            station.filename),
                   fontweight='bold')
